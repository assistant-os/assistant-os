...
class Middleware extends EventEmitter {
  ...
  */
  ...
  getPathId () {
    if (this.parent) {
      return `${this.parent.getPathId()}/${this.id}`
    } else {
      return `${this.id}`
    }
  }
  ...
  listRecursively (index, callback, endCallback) {
    ...
    const next = () => {
      this.listRecursively(index + 1, callback, endCallback)
    }
  ...
  list (callback, endCallback) {
    callback && callback(this, () => {
      this.listRecursively(0, callback, endCallback)
    })
  }
